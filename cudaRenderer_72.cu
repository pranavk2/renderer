...
//	__shared__ volatile uint temp[NUM_THREADS<<1];
	...
	int numCircles = cuConstRendererParams.numCircles ;
	int numCirclesPerThread = numCircles / NUM_THREADS;
	uint circles_this_thread[100000 / NUM_THREADS];
	int ctr = 0;
	...
	for (int i=tid*numCirclesPerThread; i < (tid+1)*numCirclesPerThread; i++)
	{
		circleIndex = i;
	...
	__shared__ uint circles_this_block[100000];
	for (int i=s_out[tid]; i<s_out[tid+1]; i++)
	{
		circles_this_block[i] = circles_this_thread[i-s_out[tid]];
	}
	__syncthreads();
	for (int i=0; i < circlesThisBlock; i++)
	{
		...
		{
			circleIndexFinal = circles_this_block[i];
