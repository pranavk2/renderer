...
#define CIRCLES_PER_THREAD_PER_ITERATION 5
////////////////////////////////////////////////////////////////////////////////////////
...
__global__ void kernelRenderWindowOfPixelsNaive() {
	...
	// one thread goes for one pixel and one block operates on a group of pixels
	int pixelX = tid_x;
	int pixelY = tid_y;	
...
__global__ void kernelRenderWindowOfPixelsOpt1(int windowsAlongWidth, int windowsAlongHeight) {
	...
	float invWidth = 1.f / imageWidth;
	float invHeight = 1.f / imageHeight;
	__shared__ uint16_t s[NUM_THREADS];
	__shared__ uint16_t s_out[NUM_THREADS];
	float3 p, pFinal;		
	float rad, radFinal;
	float4* imgPtr;
	...
	int numCircles = cuConstRendererParams.numCircles ;
	__shared__ uint32_t circles_this_thread[CIRCLES_PER_THREAD_PER_ITERATION*NUM_THREADS];
	//uint32_t circles_this_thread[CIRCLES_PER_THREAD_PER_ITERATION];
	int ctr = 0;
	...
	__shared__ uint32_t circles_this_block_per_iteration[CIRCLES_PER_THREAD_PER_ITERATION*NUM_THREADS];

	for (int numCircle=0; numCircle < numCircles; numCircle += (CIRCLES_PER_THREAD_PER_ITERATION*NUM_THREADS)) 
	{
		ctr = 0;	
		for (int i=numCircle+tid*CIRCLES_PER_THREAD_PER_ITERATION; (i < numCircle+(tid+1)*CIRCLES_PER_THREAD_PER_ITERATION && i < numCircles) ; i++)
		{
			circleIndex = i;
			...
			if (circleInBoxConservative(p.x*imageWidth, p.y*imageHeight, rad*imageHeight, windowLeft, windowRight, windowBottom, windowTop))
			{
				circles_this_thread[tid*CIRCLES_PER_THREAD_PER_ITERATION+(ctr++)] = circleIndex;
	//			circles_this_thread[(ctr++)] = circleIndex;
			}
		}
		s[tid] = ctr;
		s_out[tid] = s[tid];
		...
		int circlesThisBlock = s[NUM_THREADS-1] + s_out[NUM_THREADS-1];
		if (tid < NUM_THREADS-1) 
		{
			for (int i=s_out[tid]; i<s_out[tid+1]; i++)
			{
	//			circles_this_block_per_iteration[i] = circles_this_thread[i-s_out[tid]];
				circles_this_block_per_iteration[i] = circles_this_thread[  tid*CIRCLES_PER_THREAD_PER_ITERATION+i-s_out[tid]];
			}
		}
		else
		{
			for (int i=0; i<s[tid]; i++)
			{
				//circles_this_block_per_iteration[i+s_out[tid]] = circles_this_thread[i  ];
				circles_this_block_per_iteration[i+s_out[tid]] = circles_this_thread[i+ tid*CIRCLES_PER_THREAD_PER_ITERATION  ];
			}
		}
		__syncthreads();
		for (int i=0; i < circlesThisBlock; i++)
		{

			if (pixelX < imageWidth && pixelY < imageHeight)
			{
				circleIndexFinal = circles_this_block_per_iteration[i];
				...
				if (pixelDist > maxDist)
					...
					shadePixel(circleIndexFinal, pixelCenter, pFinal, &rgb);
